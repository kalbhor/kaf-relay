...
func (h *consumerHook) OnBrokerDisconnect(meta kgo.BrokerMetadata, conn net.Conn) {
	...
	// Client could be nil if hook is called before/during boot (initialising clients)
	// these disconnect calls need to be ignored.
	...
	h.m.c.logger.Info("on broker disconnect called", "host", meta.Host, "port", meta.Port)
